<template>
  <div class="max-w-4xl mx-auto py-10">
    <h1 class="text-2xl font-bold mb-6">Buscar Gastos</h1>
      ...
      <table class="w-full table-auto border rounded-lg bg-white shadow">
        ...
        <tbody>
          <tr v-for="gasto in gastosFiltrados" :key="gasto._id">
            <td class="px-4 py-2">{{ gasto.descripcion || gasto.title || gasto.titulo }}</td>
          ...
          <tr v-if="gastosFiltrados.length === 0">
            <td colspan="4" class="text-center py-4 text-gray-500">No se encontraron gastos.</td>
          </tr>
        </tbody>
      </table>
    </div>
